...
from share import Tr, Tp, epochs, batch_size, threshold, devices, runs
...
result_file = 'results/' + scenario + '/AGCRN-results.xlsx'
...
def train(run, runs):
    ...
    # construct the model
    net = AGCRN(num_nodes=N, dim_embed=int(0.3 * Tr + 0.7 * Tp), dim_hidden=Tp)
    net.initialize(ctx=devices, force_reinit=True)
    loss_fun = mx.gluon.loss.L2Loss()
    trainer = Trainer(net.collect_params(), 'adam')

    # writer = SummaryWriter('tensor-board')
    # input = torch.zeros((5,N,T))
    ...
    for e in range(epochs):
        ...
        print(
            f'agcrn, run {run+1}/{runs}, train, epoch {e+1}/{epochs}, duration {end-start}s, average train loss {train_loss_mean}')
    ...
    print(f'agcrn, run {run+1}/{runs}, test, duration {end-start}s, average test loss {test_loss_mean}')
...
def main():
    ...
    t.toc('agcrn, trainning finished')
