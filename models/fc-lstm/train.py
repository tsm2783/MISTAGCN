...
result_file = 'results/' + scenario + '/FC-LSTM-results.xlsx'
...
def train(run, runs):
    ...
    # file=open('results/Haikou/lstm-fc-idices.txt','w')
    ...
    for e in range(epochs):
        ...
        print(
            f'lstm-fc, run {run+1}/{runs}, train, epoch {e+1}/{epochs}, duration {end-start}s, average train loss {train_loss_mean}')
    ...
    print(f'lstm-fc, run {run+1}/{runs}, test, duration {end-start}s, average test loss {test_loss_mean}')
...
def main():
    ...
    t.toc('lstm-fc, trainning finished')
