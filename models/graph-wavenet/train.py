...
from share import batch_size, devices, epochs, runs, threshold
...
result_file = 'results/' + scenario + '/Graph WaveNet-results.xlsx'
...
def train(run, runs):
    ...
    for e in range(epochs):
        ...
        print(
            f'graph-wavenet, run {run+1}/{runs}, train, epoch {e+1}/{epochs}, duration {end-start}s, average train loss {train_loss_mean}')
    ...
    print(f'graph-wavenet, run {run+1}/{runs}, test, duration {end-start}s, average test loss {test_loss_mean}')
...
def main():
    ...
    t.toc('graph-wavenet, trainning finished')
