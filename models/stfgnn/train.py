...
scenario = 'Haikou'  # Haikou, YellowTrip
# batch_size = 100
# runs = 2
# epochs = 5

data_sample_file = './data/' + scenario + '/data-samples'
adj_s_file = './data/' + scenario + '/adg'
adj_t_file = './data/' + scenario + '/adj-temporal-sparse'
result_file = 'results/' + scenario + '/STFGNN-results.xlsx'

Xr_sample, Xd_sample, Xw_sample, Yp_sample, regions = nd.load(data_sample_file)
ADJ_s, = nd.load(adj_s_file)
ADJ_t, = nd.load(adj_t_file)
...
def train(run, runs):
    ...
    X = nd.transpose(Xr_sample, axes=(0, 3, 1, 2))  #B, T, N, C
    Y = nd.transpose(Yp_sample, axes=(0, 2, 1))  #B, T', N
    ...
    # construct the model
    net = STFGNN(ADJ_s, ADJ_t, input_length=Tr, num_vertices=N, predict_length=Tp)
    ...
    for e in range(epochs):
        ...
        print(
            f'stfgnn, run {run+1}/{runs}, train, epoch {e+1}/{epochs}, duration {end-start}s, average train loss {train_loss_mean}')
    ...
    print(f'stfgnn, run {run+1}/{runs}, test, duration {end-start}s, average test loss {test_loss_mean}')
...
def main():
    ...
    t.toc('stfgnn, trainning finished')
